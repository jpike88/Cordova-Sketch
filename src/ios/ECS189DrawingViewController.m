...
- (void)viewDidLoad
{
    ...
    if(IS_IPAD){
        ...
        _scrollView.minimumZoomScale = finalScale;
    ...
    if([[_sketchView returnSketchArray] count] == 0){
        [_stampsButton setHidden:YES];
    }
    ...
    if(_collectionRedoQueue == nil) {
        _collectionRedoQueue = [[NSMutableArray alloc] init];
    }
...
self.undoButton.layer.borderWidth = 0.0f;
    ...
    [self.compassButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    
    [self.compassButton setTitleColor:[UIColor colorWithRed:40 green:41 blue:65 alpha:1] forState:UIControlStateHighlighted];
    [self.compassButton setTitle:self.labelForCompass forState:UIControlStateNormal];
    [self.compassLabel setText:self.labelForCompass];
...
-(IBAction)exitFullScreen:(id)sender{
    ...
    _sketchView.result = [CDVPluginResult
